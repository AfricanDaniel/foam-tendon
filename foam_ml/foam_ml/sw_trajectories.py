# %%
"""
Visualise all move_SW_ trajectories from training data.

Three views:
  1. Top-down   (North vs East)  — SW moves show negative East, negative North
  2. Side view  (SW diagonal magnitude vs Up)
  3. 3D         (East, North, Up)

Plus motor panels (all four active during diagonal moves):
  4. All motors vs SW diagonal magnitude
  5. All motors vs Up displacement

SW is the negative-X, negative-Y direction in the robot frame.
  diag_mm = sqrt(east_mm^2 + north_mm^2)  — horizontal distance from home

Motor behaviour during a SW move:
...
print(f"Found {len(move_sw_files)} move_SW_ files")

# ── Load and zero each trajectory at home ────────────────────────────────────
trajectories = []

for f in move_sw_files:
    m = re.search(r'move_SW_(\d+\.?\d*)deg', f.name)
...
fig1.update_layout(
    title='move_SW_ Trajectories — Top-Down View<br>'
          '<sup>South vs West displacement from home</sup>',
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 2 — Side view  (SW diagonal magnitude vs Up)
# ─────────────────────────────────────────────────────────────────────────────
# %%
fig2 = go.Figure()
...
fig2.update_layout(
    title='move_SW_ Trajectories — Side View<br>'
          '<sup>Up displacement vs SW diagonal magnitude from home</sup>',
    xaxis_title='SW Diagonal Magnitude (mm)',
...
fig3.update_layout(
    title='move_SW_ Trajectories — 3D View',
    scene=dict(
        xaxis_title='West (mm)',
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 4 — All motor positions vs SW diagonal magnitude
# ─────────────────────────────────────────────────────────────────────────────
# %%
fig4 = make_subplots(
    rows=2, cols=2,
    subplot_titles=['Motor 1 (North release) vs SW', 'Motor 2 (East release) vs SW',
                    'Motor 3 (South pull) vs SW',    'Motor 4 (West pull) vs SW'],
    shared_xaxes=False,
)
...
fig4.update_layout(
    title='Motor Positions vs SW Diagonal Magnitude<br>'
          '<sup>Pulses from home — all move_SW_ trajectories</sup>',
    template='plotly_white',
    legend_title='Amplitude',
    width=950, height=750,
)
for r, c in positions:
    fig4.update_xaxes(title_text='SW Diagonal Magnitude (mm)', row=r, col=c)
...
fig5 = make_subplots(
    rows=2, cols=2,
    subplot_titles=['Motor 1 (North release) vs Up', 'Motor 2 (East release) vs Up',
                    'Motor 3 (South pull) vs Up',    'Motor 4 (West pull) vs Up'],
    shared_xaxes=False,
)
