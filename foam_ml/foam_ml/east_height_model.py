# %%
"""
Model: East displacement ↔ Up displacement (foam height) ↔ Motor positions

Trains two models on all move_E_ trajectory data:
  1. Linear Regression   (baseline)
  2. Neural Network MLP  (sklearn)

Active motors for East moves: Motor 2 (East pull) and Motor 4 (West release).
Both decrease (go CW) during an East move.

Sections:
  A. East (mm)  → Up (mm)              "given how far east, predict height"
  B. Up (mm)    → East (mm)            "given height, predict east position"
  C. East (mm)  → Motor 2 & 4 (pulses) "given east, predict motor commands"
...
for f in move_e_files:
    ...
    if valid.empty:
        continue
    home = valid.iloc[0]
    east_all.extend((valid['optitrack_x'] - home['optitrack_x']) * 1000)   # mm
...
east = np.array(east_all)
up   = np.array(up_all)
m2   = np.array(m2_all, dtype=float)
m4   = np.array(m4_all, dtype=float)
print(f"Loaded {len(east):,} data points from {len(move_e_files)} files")
print(f"East    range: {east.min():.1f} mm  →  {east.max():.1f} mm")
...
X_e = east.reshape(-1, 1)
X_u = up.reshape(-1, 1)

# Direction A: East → Up
Xe_tr, Xe_te, yu_tr, yu_te = train_test_split(X_e, up, test_size=0.2, random_state=42)

# Direction B: Up → East
Xu_tr, Xu_te, ye_tr, ye_te = train_test_split(X_u, east, test_size=0.2, random_state=42)
...
lr_a = LinearRegression().fit(Xe_tr, yu_tr)
lr_a_pred = lr_a.predict(Xe_te)
...
nn_a = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
    ...
    sc_e.transform(Xe_tr),
    sc_yu.transform(yu_tr.reshape(-1, 1)).ravel()
)
nn_a_pred_sc = nn_a.predict(sc_e.transform(Xe_te))
...
e_grid = np.linspace(east.min(), east.max(), 300).reshape(-1, 1)
lr_curve_a = lr_a.predict(e_grid)
nn_curve_a = sc_yu.inverse_transform(
    nn_a.predict(sc_e.transform(e_grid)).reshape(-1, 1)
).ravel()

fig_a = go.Figure()
fig_a.add_trace(go.Scatter(
    x=east, y=up, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3),
    name='Data'
))
fig_a.add_trace(go.Scatter(
    x=e_grid.ravel(), y=lr_curve_a, mode='lines',
    line=dict(color='crimson', width=2, dash='dash'),
    name=f'Linear  R²={lr_a_r2:.3f}  RMSE={lr_a_rmse:.1f} mm'
))
fig_a.add_trace(go.Scatter(
    x=e_grid.ravel(), y=nn_curve_a, mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_a_r2:.3f}  RMSE={nn_a_rmse:.1f} mm'
))
fig_a.update_layout(
    title='East → Up: Model Fit<br>'
          '<sup>Given east displacement, predict foam height (Up)</sup>',
...
lr_b = LinearRegression().fit(Xu_tr, ye_tr)
lr_b_pred = lr_b.predict(Xu_te)
lr_b_r2   = r2_score(ye_te, lr_b_pred)
lr_b_rmse = np.sqrt(mean_squared_error(ye_te, lr_b_pred))

sc_u  = StandardScaler().fit(Xu_tr)
sc_ye = StandardScaler().fit(ye_tr.reshape(-1, 1))
nn_b = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_u.transform(Xu_tr),
    sc_ye.transform(ye_tr.reshape(-1, 1)).ravel()
)
nn_b_pred_sc = nn_b.predict(sc_u.transform(Xu_te))
nn_b_pred    = sc_ye.inverse_transform(nn_b_pred_sc.reshape(-1, 1)).ravel()
nn_b_r2      = r2_score(ye_te, nn_b_pred)
nn_b_rmse    = np.sqrt(mean_squared_error(ye_te, nn_b_pred))
...
nn_curve_b = sc_ye.inverse_transform(
    nn_b.predict(sc_u.transform(u_grid)).reshape(-1, 1)
).ravel()

fig_b = go.Figure()
fig_b.add_trace(go.Scatter(
    x=up, y=east, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3),
    name='Data'
))
fig_b.add_trace(go.Scatter(
...
fig_b.update_layout(
    title='Up → East: Model Fit<br>'
          '<sup>Given foam height (Up), predict east displacement</sup>',
    xaxis_title='Up Displacement / Height (mm)',
    yaxis_title='East Displacement (mm)',
    template='plotly_white',
    legend=dict(x=0.02, y=0.98),
    width=800, height=550,
)
...
fig_r = make_subplots(
    rows=1, cols=2,
    subplot_titles=[
        'East → Up: Predicted vs Actual',
        'Up → East: Predicted vs Actual',
    ]
)
...
for pred, label, color in [
    ...
    fig_r.add_trace(go.Scatter(
        x=ye_te, y=pred, mode='markers',
        marker=dict(size=3, color=color, opacity=0.5),
        name=label, legendgroup=label, showlegend=False,
    ), row=1, col=2)

for col, (mn, mx) in enumerate(
    [(yu_te.min(), yu_te.max()), (ye_te.min(), ye_te.max())], start=1
):
    fig_r.add_trace(go.Scatter(
...
fig_r.update_xaxes(title_text='Actual East (mm)',    row=1, col=2)
fig_r.update_yaxes(title_text='Predicted East (mm)', row=1, col=2)
...
# ─────────────────────────────────────────────────────────────────────────────
# Direction C: East → Motor 2 & 4
# ─────────────────────────────────────────────────────────────────────────────
# %%

motors_24 = np.column_stack([m2, m4])

Xe_tr_c, Xe_te_c, ym_tr_c, ym_te_c = train_test_split(
    X_e, motors_24, test_size=0.2, random_state=42
)

lr_c = LinearRegression().fit(Xe_tr_c, ym_tr_c)
lr_c_pred = lr_c.predict(Xe_te_c)
...
sc_e_c = StandardScaler().fit(Xe_tr_c)
sc_mc  = StandardScaler().fit(ym_tr_c)
nn_c = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_e_c.transform(Xe_tr_c),
    sc_mc.transform(ym_tr_c)
)
nn_c_pred = sc_mc.inverse_transform(nn_c.predict(sc_e_c.transform(Xe_te_c)))
...
lr_curves_c = lr_c.predict(e_grid)
nn_curves_c = sc_mc.inverse_transform(nn_c.predict(sc_e_c.transform(e_grid)))

fig_c = make_subplots(rows=1, cols=2,
                      subplot_titles=['East → Motor 2', 'East → Motor 4'])

for col_idx, (motor_data, lbl) in enumerate([(m2, 'Motor 2'), (m4, 'Motor 4')], start=1):
    fig_c.add_trace(go.Scatter(
        x=east, y=motor_data, mode='markers',
        marker=dict(size=2, color='lightsteelblue', opacity=0.3),
        name='Data', showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.add_trace(go.Scatter(
        x=e_grid.ravel(), y=lr_curves_c[:, col_idx - 1], mode='lines',
        line=dict(color='crimson', width=2, dash='dash'),
        name=f'Linear R²={lr_c_r2[col_idx-1]:.3f} RMSE={lr_c_rmse[col_idx-1]:.0f} p',
        showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.add_trace(go.Scatter(
        x=e_grid.ravel(), y=nn_curves_c[:, col_idx - 1], mode='lines',
        line=dict(color='darkorange', width=2),
        name=f'MLP    R²={nn_c_r2[col_idx-1]:.3f} RMSE={nn_c_rmse[col_idx-1]:.0f} p',
        showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.update_xaxes(title_text='East Displacement (mm)', row=1, col=col_idx)
    fig_c.update_yaxes(title_text=f'{lbl} Position (pulses from home)', row=1, col=col_idx)

fig_c.update_layout(
    title='East → Motor Positions: Model Fit<br>'
          '<sup>Given east displacement, predict Motor 2 & Motor 4 pulses from home</sup>',
    template='plotly_white',
