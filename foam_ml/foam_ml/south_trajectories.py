# %%
"""
Visualise all move_S_ trajectories from training data.

Three views:
  1. Top-down   (North vs East)  — South moves show negative North values
  2. Side view  (South vs Up)    — South displacement = −optitrack_y
  3. 3D         (East, North, Up)

Plus a motor-position panel showing how Motors 1 & 3 change with South displacement.
South is the negative-Y direction in the robot frame, so south_mm = −optitrack_y × 1000
(positive = further south). Motors 1 (North, release) and 3 (South, pull) both move
CCW (positive pulses) during a south move.
...
print(f"Found {len(move_s_files)} move_S_ files")

# ── Load and zero each trajectory at home ────────────────────────────────────
trajectories = []

for f in move_s_files:
    m = re.search(r'move_S_(\d+\.?\d*)deg', f.name)
    ...
    df['south_mm'] = -df['optitrack_y'] * 1000   # positive = further south
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 1 — Top-down view  (South vs East)
# ─────────────────────────────────────────────────────────────────────────────
# %%
fig1 = go.Figure()
...
fig1.update_layout(
    title='move_S_ Trajectories — Top-Down View<br>'
          '<sup>South vs East displacement from home</sup>',
...
fig2.update_layout(
    title='move_S_ Trajectories — Side View<br>'
          '<sup>Up displacement vs South displacement from home</sup>',
...
fig3.update_layout(
    title='move_S_ Trajectories — 3D View',
    scene=dict(
        xaxis_title='East (mm)',
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 4 — Motor 1 & 3 positions vs South displacement
# ─────────────────────────────────────────────────────────────────────────────
# %%
...
fig4 = make_subplots(
    rows=1, cols=2,
    subplot_titles=['Motor 1 vs South', 'Motor 3 vs South'],
    shared_xaxes=False,
)
...
fig4.update_layout(
    title='Motor Positions vs South Displacement<br>'
          '<sup>Pulses from home — all move_S_ trajectories (Motor 1 = North release, Motor 3 = South pull)</sup>',
    template='plotly_white',
    legend_title='Amplitude',
    width=950, height=500,
)
for r, c in positions:
    fig4.update_xaxes(title_text='South Displacement (mm)', row=r, col=c)
