# %%
"""
Model: NW diagonal displacement ↔ Up displacement (foam height) ↔ Motor positions

Trains two models on all move_NW_ trajectory data:
  1. Linear Regression   (baseline)
  2. Neural Network MLP  (sklearn)

NW displacement = sqrt(east_mm^2 + north_mm^2)  — horizontal magnitude from home.
All four motors are active during NW moves:
...
Sections:
  A. NW (mm) → Up (mm)                 "given diagonal distance, predict height"
  B. Up (mm) → NW (mm)                 "given height, predict diagonal distance"
  C. NW (mm) → Motors 1, 2, 3, 4      "given NW distance, predict all motor commands"
...
nw_all, up_all, m1_all, m2_all, m3_all, m4_all = [], [], [], [], [], []
...
for f in move_nw_files:
    ...
    nw_all.extend(np.sqrt(east**2 + north**2))               # mm magnitude
...
print(f"Loaded {len(nw):,} data points from {len(move_nw_files)} files")
print(f"NW diag range: {nw.min():.1f} mm  →  {nw.max():.1f} mm")
...
Xnw_tr, Xnw_te, yu_tr, yu_te = train_test_split(X_nw, up,  test_size=0.2, random_state=42)
Xu_tr,  Xu_te,  ynw_tr,ynw_te= train_test_split(X_u,  nw,  test_size=0.2, random_state=42)

# ─────────────────────────────────────────────────────────────────────────────
# Direction A: NW → Up
# ─────────────────────────────────────────────────────────────────────────────
# %%
lr_a = LinearRegression().fit(Xnw_tr, yu_tr)
lr_a_pred = lr_a.predict(Xnw_te)
lr_a_r2   = r2_score(yu_te, lr_a_pred)
lr_a_rmse = np.sqrt(mean_squared_error(yu_te, lr_a_pred))

sc_nw = StandardScaler().fit(Xnw_tr)
...
nn_a = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
    ...
    sc_nw.transform(Xnw_tr), sc_yu.transform(yu_tr.reshape(-1,1)).ravel())
nn_a_pred = sc_yu.inverse_transform(
    nn_a.predict(sc_nw.transform(Xnw_te)).reshape(-1,1)).ravel()
...
nw_grid = np.linspace(nw.min(), nw.max(), 300).reshape(-1, 1)
fig_a = go.Figure()
fig_a.add_trace(go.Scatter(x=nw, y=up, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3), name='Data'))
fig_a.add_trace(go.Scatter(x=nw_grid.ravel(), y=lr_a.predict(nw_grid), mode='lines',
    line=dict(color='crimson', width=2, dash='dash'),
    name=f'Linear  R²={lr_a_r2:.3f}  RMSE={lr_a_rmse:.1f} mm'))
fig_a.add_trace(go.Scatter(x=nw_grid.ravel(), y=sc_yu.inverse_transform(
    nn_a.predict(sc_nw.transform(nw_grid)).reshape(-1,1)).ravel(), mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_a_r2:.3f}  RMSE={nn_a_rmse:.1f} mm'))
fig_a.update_layout(
    title='NW → Up: Model Fit<br><sup>Given NW diagonal distance, predict foam height</sup>',
    xaxis_title='NW Diagonal Magnitude (mm)', yaxis_title='Up Displacement / Height (mm)',
...
# ─────────────────────────────────────────────────────────────────────────────
# %%
lr_b = LinearRegression().fit(Xu_tr, ynw_tr)
lr_b_pred = lr_b.predict(Xu_te)
lr_b_r2   = r2_score(ynw_te, lr_b_pred)
lr_b_rmse = np.sqrt(mean_squared_error(ynw_te, lr_b_pred))

sc_u  = StandardScaler().fit(Xu_tr)
sc_ynw = StandardScaler().fit(ynw_tr.reshape(-1, 1))
nn_b = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_u.transform(Xu_tr), sc_ynw.transform(ynw_tr.reshape(-1,1)).ravel())
nn_b_pred = sc_ynw.inverse_transform(
    nn_b.predict(sc_u.transform(Xu_te)).reshape(-1,1)).ravel()
nn_b_r2   = r2_score(ynw_te, nn_b_pred)
nn_b_rmse = np.sqrt(mean_squared_error(ynw_te, nn_b_pred))
...
fig_b = go.Figure()
fig_b.add_trace(go.Scatter(x=up, y=nw, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3), name='Data'))
...
fig_b.add_trace(go.Scatter(x=u_grid.ravel(), y=sc_ynw.inverse_transform(
    nn_b.predict(sc_u.transform(u_grid)).reshape(-1,1)).ravel(), mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_b_r2:.3f}  RMSE={nn_b_rmse:.1f} mm'))
fig_b.update_layout(
    title='Up → NW: Model Fit<br><sup>Given foam height, predict NW diagonal distance</sup>',
    xaxis_title='Up Displacement / Height (mm)', yaxis_title='NW Diagonal Magnitude (mm)',
    template='plotly_white', legend=dict(x=0.02, y=0.98), width=800, height=550)
fig_b.show()
...
fig_r = make_subplots(rows=1, cols=2,
    subplot_titles=['NW → Up: Predicted vs Actual', 'Up → NW: Predicted vs Actual'])
...
for pred, label, color in [(lr_b_pred, f'Linear R²={lr_b_r2:.3f}', 'crimson'),
    ...
    fig_r.add_trace(go.Scatter(x=ynw_te, y=pred, mode='markers',
        marker=dict(size=3, color=color, opacity=0.5),
        name=label, legendgroup=label, showlegend=False), row=1, col=2)
for col, (mn, mx) in enumerate([(yu_te.min(),yu_te.max()),(ynw_te.min(),ynw_te.max())], 1):
...
fig_r.update_xaxes(title_text='Actual NW (mm)', row=1, col=2)
fig_r.update_yaxes(title_text='Predicted NW (mm)', row=1, col=2)
fig_r.update_layout(title='Predicted vs Actual — Test Set',
    template='plotly_white', width=1000, height=500)
fig_r.show()

# ─────────────────────────────────────────────────────────────────────────────
# Direction C: NW → All 4 motors
# ─────────────────────────────────────────────────────────────────────────────
# %%
motors_all = np.column_stack([m1, m2, m3, m4])
Xnw_tr_c, Xnw_te_c, ym_tr_c, ym_te_c = train_test_split(
    X_nw, motors_all, test_size=0.2, random_state=42)

lr_c = LinearRegression().fit(Xnw_tr_c, ym_tr_c)
lr_c_pred = lr_c.predict(Xnw_te_c)
...
sc_nw_c = StandardScaler().fit(Xnw_tr_c)
sc_mc   = StandardScaler().fit(ym_tr_c)
nn_c = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_nw_c.transform(Xnw_tr_c), sc_mc.transform(ym_tr_c))
nn_c_pred = sc_mc.inverse_transform(nn_c.predict(sc_nw_c.transform(Xnw_te_c)))
...
lr_curves_c = lr_c.predict(nw_grid)
nn_curves_c = sc_mc.inverse_transform(nn_c.predict(sc_nw_c.transform(nw_grid)))

motor_data_list = [m1, m2, m3, m4]
motor_labels = ['Motor 1 (N pull)', 'Motor 2 (E release)',
                'Motor 3 (S release)', 'Motor 4 (W pull)']
fig_c = make_subplots(rows=2, cols=2,
    subplot_titles=[f'NW → {l}' for l in motor_labels])
for idx, (motor_data, lbl) in enumerate(zip(motor_data_list, motor_labels)):
    r, c = divmod(idx, 2)
    row, col = r+1, c+1
    fig_c.add_trace(go.Scatter(x=nw, y=motor_data, mode='markers',
        marker=dict(size=2, color='lightsteelblue', opacity=0.3),
        name='Data', showlegend=(idx==0)), row=row, col=col)
    fig_c.add_trace(go.Scatter(x=nw_grid.ravel(), y=lr_curves_c[:,idx], mode='lines',
        line=dict(color='crimson', width=2, dash='dash'),
        name=f'Linear R²={lr_c_r2[idx]:.3f}', showlegend=(idx==0)), row=row, col=col)
    fig_c.add_trace(go.Scatter(x=nw_grid.ravel(), y=nn_curves_c[:,idx], mode='lines',
        line=dict(color='darkorange', width=2),
        name=f'MLP R²={nn_c_r2[idx]:.3f}', showlegend=(idx==0)), row=row, col=col)
    fig_c.update_xaxes(title_text='NW Diagonal Magnitude (mm)', row=row, col=col)
    fig_c.update_yaxes(title_text='Motor Position (pulses from home)', row=row, col=col)
fig_c.update_layout(
    title='NW → Motor Positions: Model Fit<br>'
          '<sup>Given NW diagonal distance, predict all motor pulses from home</sup>',
...
fig_d.update_layout(
    title='Up (Height) → Motor Positions: Model Fit<br>'
          '<sup>Given foam height, predict all motor pulses from home (move_NW_ data)</sup>',
    template='plotly_white', legend=dict(x=0.01, y=0.99), width=1000, height=750)
fig_d.show()
