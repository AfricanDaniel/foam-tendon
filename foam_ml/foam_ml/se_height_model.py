# %%
"""
Model: SE diagonal displacement ↔ Up displacement (foam height) ↔ Motor positions

Trains two models on all move_SE_ trajectory data:
  1. Linear Regression   (baseline)
  2. Neural Network MLP  (sklearn)

SE displacement = sqrt(east_mm^2 + north_mm^2)  — horizontal magnitude from home.
All four motors are active during SE moves:
...
Sections:
  A. SE (mm) → Up (mm)                 "given diagonal distance, predict height"
  B. Up (mm) → SE (mm)                 "given height, predict diagonal distance"
  C. SE (mm) → Motors 1, 2, 3, 4      "given SE distance, predict all motor commands"
...
move_se_files = sorted(data_dir.glob("*move_SE_*.csv"))

NEEDED_COLS = ['optitrack_x', 'optitrack_y', 'optitrack_z',
               'motor_1_pos', 'motor_2_pos', 'motor_3_pos', 'motor_4_pos']
se_all, up_all, m1_all, m2_all, m3_all, m4_all = [], [], [], [], [], []

for f in move_se_files:
    df = pd.read_csv(f)
    ...
    se_all.extend(np.sqrt(east**2 + north**2))
...
print(f"Loaded {len(se):,} data points from {len(move_se_files)} files")
print(f"SE diag range: {se.min():.1f} mm  →  {se.max():.1f} mm")
...
Xse_tr, Xse_te, yu_tr, yu_te   = train_test_split(X_se, up, test_size=0.2, random_state=42)
Xu_tr,  Xu_te,  yse_tr, yse_te = train_test_split(X_u,  se, test_size=0.2, random_state=42)

# ─────────────────────────────────────────────────────────────────────────────
# Direction A: SE → Up
# ─────────────────────────────────────────────────────────────────────────────
# %%
lr_a = LinearRegression().fit(Xse_tr, yu_tr)
lr_a_pred = lr_a.predict(Xse_te)
lr_a_r2   = r2_score(yu_te, lr_a_pred)
lr_a_rmse = np.sqrt(mean_squared_error(yu_te, lr_a_pred))

sc_se = StandardScaler().fit(Xse_tr)
...
nn_a = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
    ...
    sc_se.transform(Xse_tr), sc_yu.transform(yu_tr.reshape(-1,1)).ravel())
nn_a_pred = sc_yu.inverse_transform(
    nn_a.predict(sc_se.transform(Xse_te)).reshape(-1,1)).ravel()
...
se_grid = np.linspace(se.min(), se.max(), 300).reshape(-1, 1)
fig_a = go.Figure()
fig_a.add_trace(go.Scatter(x=se, y=up, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3), name='Data'))
fig_a.add_trace(go.Scatter(x=se_grid.ravel(), y=lr_a.predict(se_grid), mode='lines',
    line=dict(color='crimson', width=2, dash='dash'),
    name=f'Linear  R²={lr_a_r2:.3f}  RMSE={lr_a_rmse:.1f} mm'))
fig_a.add_trace(go.Scatter(x=se_grid.ravel(), y=sc_yu.inverse_transform(
    nn_a.predict(sc_se.transform(se_grid)).reshape(-1,1)).ravel(), mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_a_r2:.3f}  RMSE={nn_a_rmse:.1f} mm'))
fig_a.update_layout(
    title='SE → Up: Model Fit<br><sup>Given SE diagonal distance, predict foam height</sup>',
    xaxis_title='SE Diagonal Magnitude (mm)', yaxis_title='Up Displacement / Height (mm)',
...
# ─────────────────────────────────────────────────────────────────────────────
# %%
lr_b = LinearRegression().fit(Xu_tr, yse_tr)
lr_b_pred = lr_b.predict(Xu_te)
lr_b_r2   = r2_score(yse_te, lr_b_pred)
lr_b_rmse = np.sqrt(mean_squared_error(yse_te, lr_b_pred))

sc_u   = StandardScaler().fit(Xu_tr)
sc_yse = StandardScaler().fit(yse_tr.reshape(-1, 1))
nn_b = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_u.transform(Xu_tr), sc_yse.transform(yse_tr.reshape(-1,1)).ravel())
nn_b_pred = sc_yse.inverse_transform(
    nn_b.predict(sc_u.transform(Xu_te)).reshape(-1,1)).ravel()
nn_b_r2   = r2_score(yse_te, nn_b_pred)
nn_b_rmse = np.sqrt(mean_squared_error(yse_te, nn_b_pred))
...
fig_b = go.Figure()
fig_b.add_trace(go.Scatter(x=up, y=se, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3), name='Data'))
...
fig_b.add_trace(go.Scatter(x=u_grid.ravel(), y=sc_yse.inverse_transform(
    nn_b.predict(sc_u.transform(u_grid)).reshape(-1,1)).ravel(), mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_b_r2:.3f}  RMSE={nn_b_rmse:.1f} mm'))
fig_b.update_layout(
    title='Up → SE: Model Fit<br><sup>Given foam height, predict SE diagonal distance</sup>',
    xaxis_title='Up Displacement / Height (mm)', yaxis_title='SE Diagonal Magnitude (mm)',
    template='plotly_white', legend=dict(x=0.02, y=0.98), width=800, height=550)
fig_b.show()
...
fig_r = make_subplots(rows=1, cols=2,
    subplot_titles=['SE → Up: Predicted vs Actual', 'Up → SE: Predicted vs Actual'])
...
for pred, label, color in [(lr_b_pred, f'Linear R²={lr_b_r2:.3f}', 'crimson'),
    ...
    fig_r.add_trace(go.Scatter(x=yse_te, y=pred, mode='markers',
        marker=dict(size=3, color=color, opacity=0.5),
        name=label, legendgroup=label, showlegend=False), row=1, col=2)
for col, (mn, mx) in enumerate([(yu_te.min(),yu_te.max()),(yse_te.min(),yse_te.max())], 1):
...
fig_r.update_xaxes(title_text='Actual SE (mm)', row=1, col=2)
fig_r.update_yaxes(title_text='Predicted SE (mm)', row=1, col=2)
fig_r.update_layout(title='Predicted vs Actual — Test Set',
    template='plotly_white', width=1000, height=500)
fig_r.show()

# ─────────────────────────────────────────────────────────────────────────────
# Direction C: SE → All 4 motors
# ─────────────────────────────────────────────────────────────────────────────
# %%
motors_all = np.column_stack([m1, m2, m3, m4])
Xse_tr_c, Xse_te_c, ym_tr_c, ym_te_c = train_test_split(
    X_se, motors_all, test_size=0.2, random_state=42)

lr_c = LinearRegression().fit(Xse_tr_c, ym_tr_c)
lr_c_pred = lr_c.predict(Xse_te_c)
...
sc_se_c = StandardScaler().fit(Xse_tr_c)
sc_mc   = StandardScaler().fit(ym_tr_c)
nn_c = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_se_c.transform(Xse_tr_c), sc_mc.transform(ym_tr_c))
nn_c_pred = sc_mc.inverse_transform(nn_c.predict(sc_se_c.transform(Xse_te_c)))
...
lr_curves_c = lr_c.predict(se_grid)
nn_curves_c = sc_mc.inverse_transform(nn_c.predict(sc_se_c.transform(se_grid)))

motor_data_list = [m1, m2, m3, m4]
motor_labels = ['Motor 1 (N release)', 'Motor 2 (E pull)',
                'Motor 3 (S pull)',    'Motor 4 (W release)']
fig_c = make_subplots(rows=2, cols=2,
    subplot_titles=[f'SE → {l}' for l in motor_labels])
for idx, (motor_data, lbl) in enumerate(zip(motor_data_list, motor_labels)):
    r, c = divmod(idx, 2)
    row, col = r+1, c+1
    fig_c.add_trace(go.Scatter(x=se, y=motor_data, mode='markers',
        marker=dict(size=2, color='lightsteelblue', opacity=0.3),
        name='Data', showlegend=(idx==0)), row=row, col=col)
    fig_c.add_trace(go.Scatter(x=se_grid.ravel(), y=lr_curves_c[:,idx], mode='lines',
        line=dict(color='crimson', width=2, dash='dash'),
        name=f'Linear R²={lr_c_r2[idx]:.3f}', showlegend=(idx==0)), row=row, col=col)
    fig_c.add_trace(go.Scatter(x=se_grid.ravel(), y=nn_curves_c[:,idx], mode='lines',
        line=dict(color='darkorange', width=2),
        name=f'MLP R²={nn_c_r2[idx]:.3f}', showlegend=(idx==0)), row=row, col=col)
    fig_c.update_xaxes(title_text='SE Diagonal Magnitude (mm)', row=row, col=col)
    fig_c.update_yaxes(title_text='Motor Position (pulses from home)', row=row, col=col)
fig_c.update_layout(
    title='SE → Motor Positions: Model Fit<br>'
          '<sup>Given SE diagonal distance, predict all motor pulses from home</sup>',
...
fig_d.update_layout(
    title='Up (Height) → Motor Positions: Model Fit<br>'
          '<sup>Given foam height, predict all motor pulses from home (move_SE_ data)</sup>',
    template='plotly_white', legend=dict(x=0.01, y=0.99), width=1000, height=750)
fig_d.show()
