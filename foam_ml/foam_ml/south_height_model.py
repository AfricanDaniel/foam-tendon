# %%
"""
Model: South displacement ↔ Up displacement (foam height) ↔ Motor positions

Trains two models on all move_S_ trajectory data:
  1. Linear Regression   (baseline)
  2. Neural Network MLP  (sklearn)

South displacement = −optitrack_y × 1000 mm  (positive = further south).
Active motors for South moves: Motor 1 (North, releases CCW) and Motor 3 (South, pulls CCW).
Both increase (go CCW / positive pulses) during a south move.

Sections:
  A. South (mm) → Up (mm)               "given how far south, predict height"
  B. Up (mm)    → South (mm)            "given height, predict south position"
  C. South (mm) → Motor 1 & 3 (pulses) "given south, predict motor commands"
...
move_s_files = sorted(data_dir.glob("*move_S_*.csv"))

NEEDED_COLS = ['optitrack_y', 'optitrack_z', 'motor_1_pos', 'motor_3_pos']
south_all, up_all, m1_all, m3_all = [], [], [], []

for f in move_s_files:
    df = pd.read_csv(f)
    valid = df[NEEDED_COLS].dropna()
    if valid.empty:
        continue
    home = valid.iloc[0]
    # Negate optitrack_y so south displacement is positive
    south_all.extend(-(valid['optitrack_y'] - home['optitrack_y']) * 1000)  # mm
...
south = np.array(south_all)
up    = np.array(up_all)
m1    = np.array(m1_all, dtype=float)
m3    = np.array(m3_all, dtype=float)
print(f"Loaded {len(south):,} data points from {len(move_s_files)} files")
print(f"South   range: {south.min():.1f} mm  →  {south.max():.1f} mm")
...
X_s = south.reshape(-1, 1)
X_u = up.reshape(-1, 1)

# Direction A: South → Up
Xs_tr, Xs_te, yu_tr, yu_te = train_test_split(X_s, up, test_size=0.2, random_state=42)

# Direction B: Up → South
Xu_tr, Xu_te, ys_tr, ys_te = train_test_split(X_u, south, test_size=0.2, random_state=42)
...
lr_a = LinearRegression().fit(Xs_tr, yu_tr)
lr_a_pred = lr_a.predict(Xs_te)
...
nn_a = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
    ...
    sc_s.transform(Xs_tr),
    sc_yu.transform(yu_tr.reshape(-1, 1)).ravel()
)
nn_a_pred_sc = nn_a.predict(sc_s.transform(Xs_te))
...
s_grid = np.linspace(south.min(), south.max(), 300).reshape(-1, 1)
lr_curve_a = lr_a.predict(s_grid)
nn_curve_a = sc_yu.inverse_transform(
    nn_a.predict(sc_s.transform(s_grid)).reshape(-1, 1)
).ravel()

fig_a = go.Figure()
fig_a.add_trace(go.Scatter(
    x=south, y=up, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3),
    name='Data'
))
fig_a.add_trace(go.Scatter(
    x=s_grid.ravel(), y=lr_curve_a, mode='lines',
    line=dict(color='crimson', width=2, dash='dash'),
    name=f'Linear  R²={lr_a_r2:.3f}  RMSE={lr_a_rmse:.1f} mm'
))
fig_a.add_trace(go.Scatter(
    x=s_grid.ravel(), y=nn_curve_a, mode='lines',
    line=dict(color='darkorange', width=2),
    name=f'MLP     R²={nn_a_r2:.3f}  RMSE={nn_a_rmse:.1f} mm'
))
fig_a.update_layout(
    title='South → Up: Model Fit<br>'
          '<sup>Given south displacement, predict foam height (Up)</sup>',
    xaxis_title='South Displacement (mm)',
...
lr_b = LinearRegression().fit(Xu_tr, ys_tr)
lr_b_pred = lr_b.predict(Xu_te)
lr_b_r2   = r2_score(ys_te, lr_b_pred)
lr_b_rmse = np.sqrt(mean_squared_error(ys_te, lr_b_pred))

sc_u  = StandardScaler().fit(Xu_tr)
sc_ys = StandardScaler().fit(ys_tr.reshape(-1, 1))
nn_b = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_u.transform(Xu_tr),
    sc_ys.transform(ys_tr.reshape(-1, 1)).ravel()
)
nn_b_pred_sc = nn_b.predict(sc_u.transform(Xu_te))
nn_b_pred    = sc_ys.inverse_transform(nn_b_pred_sc.reshape(-1, 1)).ravel()
nn_b_r2      = r2_score(ys_te, nn_b_pred)
nn_b_rmse    = np.sqrt(mean_squared_error(ys_te, nn_b_pred))
...
nn_curve_b = sc_ys.inverse_transform(
    nn_b.predict(sc_u.transform(u_grid)).reshape(-1, 1)
).ravel()

fig_b = go.Figure()
fig_b.add_trace(go.Scatter(
    x=up, y=south, mode='markers',
    marker=dict(size=2, color='lightsteelblue', opacity=0.3),
    name='Data'
))
fig_b.add_trace(go.Scatter(
...
fig_b.update_layout(
    title='Up → South: Model Fit<br>'
          '<sup>Given foam height (Up), predict south displacement</sup>',
    xaxis_title='Up Displacement / Height (mm)',
    yaxis_title='South Displacement (mm)',
    template='plotly_white',
    legend=dict(x=0.02, y=0.98),
    width=800, height=550,
)
...
fig_r = make_subplots(
    rows=1, cols=2,
    subplot_titles=[
        'South → Up: Predicted vs Actual',
        'Up → South: Predicted vs Actual',
    ]
)
...
for pred, label, color in [
    ...
    fig_r.add_trace(go.Scatter(
        x=ys_te, y=pred, mode='markers',
        marker=dict(size=3, color=color, opacity=0.5),
        name=label, legendgroup=label, showlegend=False,
    ), row=1, col=2)

for col, (mn, mx) in enumerate(
    [(yu_te.min(), yu_te.max()), (ys_te.min(), ys_te.max())], start=1
):
    fig_r.add_trace(go.Scatter(
...
fig_r.update_xaxes(title_text='Actual South (mm)',    row=1, col=2)
fig_r.update_yaxes(title_text='Predicted South (mm)', row=1, col=2)
...
# ─────────────────────────────────────────────────────────────────────────────
# Direction C: South → Motor 1 & 3
# ─────────────────────────────────────────────────────────────────────────────
# %%

motors_13 = np.column_stack([m1, m3])

Xs_tr_c, Xs_te_c, ym_tr_c, ym_te_c = train_test_split(
    X_s, motors_13, test_size=0.2, random_state=42
)

lr_c = LinearRegression().fit(Xs_tr_c, ym_tr_c)
lr_c_pred = lr_c.predict(Xs_te_c)
...
sc_s_c = StandardScaler().fit(Xs_tr_c)
sc_mc  = StandardScaler().fit(ym_tr_c)
nn_c = MLPRegressor(hidden_layer_sizes=(64, 64), activation='relu',
                    max_iter=500, random_state=42).fit(
    sc_s_c.transform(Xs_tr_c),
    sc_mc.transform(ym_tr_c)
)
nn_c_pred = sc_mc.inverse_transform(nn_c.predict(sc_s_c.transform(Xs_te_c)))
...
lr_curves_c = lr_c.predict(s_grid)
nn_curves_c = sc_mc.inverse_transform(nn_c.predict(sc_s_c.transform(s_grid)))

fig_c = make_subplots(rows=1, cols=2,
                      subplot_titles=['South → Motor 1', 'South → Motor 3'])

for col_idx, (motor_data, lbl) in enumerate([(m1, 'Motor 1'), (m3, 'Motor 3')], start=1):
    fig_c.add_trace(go.Scatter(
        x=south, y=motor_data, mode='markers',
        marker=dict(size=2, color='lightsteelblue', opacity=0.3),
        name='Data', showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.add_trace(go.Scatter(
        x=s_grid.ravel(), y=lr_curves_c[:, col_idx - 1], mode='lines',
        line=dict(color='crimson', width=2, dash='dash'),
        name=f'Linear R²={lr_c_r2[col_idx-1]:.3f} RMSE={lr_c_rmse[col_idx-1]:.0f} p',
        showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.add_trace(go.Scatter(
        x=s_grid.ravel(), y=nn_curves_c[:, col_idx - 1], mode='lines',
        line=dict(color='darkorange', width=2),
        name=f'MLP    R²={nn_c_r2[col_idx-1]:.3f} RMSE={nn_c_rmse[col_idx-1]:.0f} p',
        showlegend=(col_idx == 1),
    ), row=1, col=col_idx)
    fig_c.update_xaxes(title_text='South Displacement (mm)', row=1, col=col_idx)
    fig_c.update_yaxes(title_text=f'{lbl} Position (pulses from home)', row=1, col=col_idx)

fig_c.update_layout(
    title='South → Motor Positions: Model Fit<br>'
          '<sup>Given south displacement, predict Motor 1 & Motor 3 pulses from home</sup>',
    template='plotly_white',
...
# ─────────────────────────────────────────────────────────────────────────────
# Direction D: Up (height) → Motor 1 & 3
# ─────────────────────────────────────────────────────────────────────────────
# %%
