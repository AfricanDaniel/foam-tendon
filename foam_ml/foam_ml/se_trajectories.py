# %%
"""
Visualise all move_SE_ trajectories from training data.

Three views:
  1. Top-down   (North vs East)  — SE moves show positive East, negative North
  2. Side view  (SE diagonal magnitude vs Up)
  3. 3D         (East, North, Up)

Plus motor panels (all four active during diagonal moves):
  4. All motors vs SE diagonal magnitude
  5. All motors vs Up displacement

SE is the positive-X, negative-Y direction in the robot frame.
  diag_mm = sqrt(east_mm^2 + north_mm^2)  — horizontal distance from home

Motor behaviour during a SE move:
...
print(f"Found {len(move_se_files)} move_SE_ files")

# ── Load and zero each trajectory at home ────────────────────────────────────
trajectories = []

for f in move_se_files:
    m = re.search(r'move_SE_(\d+\.?\d*)deg', f.name)
    ...
    df['south_mm'] = -df['optitrack_y'] * 1000
...
fig1.update_layout(
    title='move_SE_ Trajectories — Top-Down View<br>'
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 2 — Side view  (SE diagonal magnitude vs Up)
# ─────────────────────────────────────────────────────────────────────────────
# %%
fig2 = go.Figure()
...
fig2.update_layout(
    title='move_SE_ Trajectories — Side View<br>'
          '<sup>Up displacement vs SE diagonal magnitude from home</sup>',
    xaxis_title='SE Diagonal Magnitude (mm)',
...
fig3.update_layout(
    title='move_SE_ Trajectories — 3D View',
    scene=dict(
        xaxis_title='East (mm)',
        yaxis_title='South (mm)',
        zaxis_title='Up (mm)',
        aspectmode='data',
    ),
    legend_title='Amplitude',
...
# ─────────────────────────────────────────────────────────────────────────────
# Plot 4 — All motor positions vs SE diagonal magnitude
# ─────────────────────────────────────────────────────────────────────────────
# %%
fig4 = make_subplots(
    rows=2, cols=2,
    subplot_titles=['Motor 1 (North release) vs SE', 'Motor 2 (East pull) vs SE',
                    'Motor 3 (South pull) vs SE',    'Motor 4 (West release) vs SE'],
    shared_xaxes=False,
)
...
fig4.update_layout(
    title='Motor Positions vs SE Diagonal Magnitude<br>'
          '<sup>Pulses from home — all move_SE_ trajectories</sup>',
    template='plotly_white',
    legend_title='Amplitude',
    width=950, height=750,
)
for r, c in positions:
    fig4.update_xaxes(title_text='SE Diagonal Magnitude (mm)', row=r, col=c)
...
fig5 = make_subplots(
    rows=2, cols=2,
    subplot_titles=['Motor 1 (North release) vs Up', 'Motor 2 (East pull) vs Up',
                    'Motor 3 (South pull) vs Up',    'Motor 4 (West release) vs Up'],
    shared_xaxes=False,
)
